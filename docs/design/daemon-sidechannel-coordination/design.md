...
> Targets `daemon_mode_b_main` (per #4175 branching strategy). Author: 秦奇. Date: 2026-05-25. Revised: 2026-05-27 (v9 — reconciliation uses an authoritative read).
...
**Contract (v9 — authoritative read, not the cache):** when a bridge model roundtrip settles, the bridge **reconciles** — it reads the agent's **true** current model via `getSessionContextStatus` (`bridge.ts:2784`, an async `extMethod` status read) and, if it diverges from the `model_switched` it just published, emits a corrective `model_switched`. **Why not the §2.3 cache:** the cache is updated only at publish sites, but a concurrent in-session change that the demux **dropped** (suppress window) was never published, so the cache can't observe it — reading the cache would falsely conclude "no divergence". The agent is the only source of truth here. The read is async, but it runs **post-settle, outside the demux**, so the §5 synchronous-block constraint (which protects the snapshot/staleness reads) does not apply. This bounds the divergence to the roundtrip window. (Longer-term alternative — routing in-session `/model` through `modelChangeQueue` — in §10.) The same reconciliation applies to A2 once `approvalModeQueue` exists.

**Observability:** every reconciliation logs `[reconcile] session=<id> trigger=roundtrip-settled published=<modelId> actual=<modelId> action=corrected|converged`. If the status read throws, log `action=read-error` and do **not** silently skip (the bus may be left diverged; surface it for oncall).
...
- **Reconciliation (§2.2, authoritative):** bridge `setSessionModel(A)` in flight → concurrent in-session `/model B` (bypasses `modelChangeQueue`) → B's notification dropped (suppress) → bridge publishes `model_switched(A)` → reconciliation's post-settle `getSessionContextStatus` read returns the agent's true model B → emits corrective `model_switched(B)`; bus converges on B. (Mock the status read to return B.) Plus a negative test: status read equals the published model → publishes nothing (`action=converged`); and a read-error test → logs `action=read-error`, no corrective.
