...
> Targets `daemon_mode_b_main` (per #4175 branching strategy). Author: 秦奇. Date: 2026-05-25. Revised: 2026-05-27 (v11 — reconciliation contract hardening).
...
**Fresh-read invariant (v11):** the `getSessionContextStatus` read used by reconciliation MUST return a fresh point-in-time value from the agent process — it MUST bypass any response cache, request deduplication, or in-flight coalescing. Without this, a cached response during the generation-guard window produces a false "converged" when no concurrent publish happened, and a stale-but-equal response produces a false "corrective" that clobbers a newer value. In practice: `extMethod` is a fresh JSON-RPC `requestSessionStatus` call on each invocation — no middleware or transport-level caching exists today. The invariant is contractual: any future caching layer MUST exempt reconciliation reads.
...
**`publishModelSwitched` helper (v11 — enforcement mechanism):** a single function that atomically (one synchronous turn): (1) sets `entry.currentModelId = modelId`, (2) increments `entry.modelPublishGeneration`, (3) publishes `model_switched` to the bus. **All** `model_switched` publish sites — bridge roundtrip success, `applyModelServiceId`, demux promotion, reconciliation corrective — MUST route through this helper. Direct `events.publish({type:'model_switched', ...})` is forbidden outside the helper. This makes it impossible to miss a generation bump, and a test invariant can assert: after any code path that produces a `model_switched`, the generation advanced by exactly 1.

**Non-recursion rule (v11):** the reconciliation corrective calls `publishModelSwitched` (a local bus publish) and does **NOT** schedule a subsequent reconciliation. Implementation must ensure the corrective path does not flow through the roundtrip-settle `.finally` hook. If an implementer factors `publishModelSwitched` through a wrapper that also attaches `.finally` reconciliation, the result is an infinite corrective loop (reconcile → read → publish → reconcile → …). Each corrective bumps the generation, but each new reconciliation reads the agent and may find divergence (the corrective updates the _bus_, not the _agent_). Guard: reconciliation is scheduled exclusively by the bridge roundtrip `.finally`; the helper itself has no side-effects beyond items (1)–(3).

**Read-error: bounded retry, then surface.** A transient `getSessionContextStatus` failure must not leave the bus permanently diverged with only a log line. Retry 1–2× with short backoff; if all fail, emit a `reconciliation_failed` bus event (so clients can warn / trigger their own status pull) and log `action=read-error`.

**Observability:** `[reconcile] session=<id> trigger=roundtrip-settled|failed published=<modelId> actual=<modelId> action=corrected|converged|skipped-newer-gen|read-error`. The reconciliation corrective is a local `publishModelSwitched` and does **not** schedule a subsequent reconciliation — implementation must ensure the corrective path does not flow through the roundtrip-settle hook.
...
- **Reconciliation (§2.2, authoritative + generation-guarded):**
  ...
  - _converged:_ status read equals the published model → no corrective (`action=converged`).
  ...
  - _read-error:_ status read fails all retries → emits `reconciliation_failed`, logs `action=read-error`, no corrective.
