...
> Targets `daemon_mode_b_main` (per #4175 branching strategy). Author: 秦奇. Date: 2026-05-25. Revised: 2026-05-26 (v8 — sixth review round: bridge state cache).
...
- **Staleness check (model):** even when no roundtrip is in flight, the demux drops a `current_model_update` promotion if a newer bridge roundtrip has already advanced `entry.currentModelId` past it (a stale late notification from a timed-out earlier change — §2 item 4). This is a synchronous read of the §2.3 state cache, not an async agent query.
...
**Contract:** when a bridge model roundtrip settles, the bridge **reconciles** — it reads the agent's current model from the **bridge state cache** (§2.3, a synchronous field read — NOT the async `extMethod` status roundtrip, which would reopen the race) and, if it diverges from the `model_switched` it just published, emits a corrective `model_switched` with the true current model. This bounds the divergence to the roundtrip window instead of leaving it permanent. (The longer-term alternative — routing in-session `/model` through `modelChangeQueue` — is noted in §10.) The same reconciliation applies to A2 approval-mode once `approvalModeQueue` exists.

**Observability:** every reconciliation logs `[reconcile] session=<id> trigger=roundtrip-settled published=<modelId> actual=<modelId> action=corrected|converged`. If the cache read is somehow absent, log `action=read-error` and do **not** silently skip (a missing cache is a bug, not a no-op).
...
- Add to `SessionEntry`: `currentModelId?: string`, `currentApprovalMode?: ApprovalMode`, `availableCommands?: …`.
- **Update synchronously at every publish site**, in the same synchronous turn as the publish (no `await` between read-of-old and write-of-new): `model_switched` (`bridge.ts:2883` / `:1172` / the demux promotion), `approval_mode_changed` (`:2979` / `:3007`), available-commands updates.
...
- **Consumers (all synchronous field reads):**
  - Staleness check: promote a `current_model_update` only if `notification.currentModelId === entry.currentModelId` would be wrong (the notification IS the new value) — instead compare against the **last bridge-published** model to detect a stale late notification; concretely, the demux drops a promotion if a newer bridge roundtrip has since updated `entry.currentModelId` past it.
  - Reconciliation: compare `entry.currentModelId` (truth) to the just-published model.
  - **`previous` derivation:** the demux fills `approval_mode_changed.previous` (and any model `previous`) from `entry.currentApprovalMode` / `entry.currentModelId` _captured before_ applying the new value — so **the agent never sends `previousModeId` / `previousModelId`** (which also sidesteps the ACP `CurrentModeUpdate` schema having no `previousModeId` field — the same external-union constraint v7 hit for A1).
  - A5 snapshot (§5): read all three fields in one synchronous block.

This makes "read current state synchronously" a first-class, single-owner mechanism rather than an ad-hoc async peek at each call site.
...
- **Staleness-drop (§2 item 4):** a stale `current_model_update(A)` arriving after the cache already advanced to B (A was the timed-out earlier change, no roundtrip in flight) is **dropped** — bus stays on B.
- **Reconciliation (§2.2):** bridge `setSessionModel(A)` in flight → concurrent in-session `/model B` (bypasses `modelChangeQueue`) → B's notification dropped (suppress) → bridge publishes `model_switched(A)` → reconciliation reads `entry.currentModelId`=B from the cache → emits corrective `model_switched(B)`; bus converges on B. Plus a negative test: reconciliation with no concurrent change publishes nothing (`action=converged`).
...
- **Bridge state cache (§2.3):** every publish site updates `entry.currentModelId` / `currentApprovalMode`; the snapshot/reconcile/staleness reads see the latest value synchronously; cache seeded on session create.
